...
void Autodromo::mostraClassificacaoCorrida() {
    ...
    for (int i = 0; i < classificacao.size(); i++) {
        cout << i + 1 << " - ID: " << classificacao[i]->getID() << " " << classificacao[i]->getMarca() << " / "
                << classificacao[i]->getPiloto()->getNome() << " (" << classificacao[i]->getPiloto()->getTipo()
                << ") - Volta-> " << classificacao[i]->getVolta() << " Posicao-> " << classificacao[i]->getPosicao() << " - Velocidade:" << classificacao[i]->getVelocidade()
                << " - Carga: ";
        std::cout << std::fixed;
